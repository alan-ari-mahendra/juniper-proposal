  ...
  try {
    ...
    if (!rateLimitResult.success) {
      return NextResponse.json(
        {
          error: `Upload rate limit exceeded. Try again in ${Math.ceil((rateLimitResult.resetTime - Date.now()) / 1000)} seconds.`,
        },
        { status: 429 },
      )
    }
    ...
    if (file.size < minSize) {
      return NextResponse.json({ error: "File too small. Minimum size is 100 bytes." }, { status: 400 })
    }
